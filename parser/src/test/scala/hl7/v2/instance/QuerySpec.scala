...
class QuerySpec extends Specification with Mocks { def is = s2"""
  ...
  Given the following elements :
    ...
    A query should succed if the path is valid                                                      $q2
    Querying a simple element should fail with unreachable path error                               $q3
    Querying an element with no children (c0) should return an empty list                           $q4
    ...
    Querying c2 for the path 4[1] and casting the result as list of `Simple' should succed          $q9
"""
...
import Query._
  ...
  def q3 = query(s0, "1[1].2[3]") must beFailedTry.withThrowable[Error]( "Unreachable Path '\\Q1[1].2[3]\\E'" )
  ...
  def q8 = queryAsSimple(c2, "2[*]") must beFailedTry.withThrowable[Error]("Path resolution returned at least one complex element")
