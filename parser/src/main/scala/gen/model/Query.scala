...
object Query {
   ...
   */
  private def children(c: Complex, position: String, instance: String): List[Element] =
    if( "*" == instance )
      c.children filter ( _.qProps.position == position.toInt )
    else c.children.filter { cc =>
      cc.qProps.position == position.toInt && cc.qProps.instance == instance.toInt
    }
}
