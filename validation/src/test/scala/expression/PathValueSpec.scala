...
trait PathValueSpec extends Specification with Evaluator with Mocks {
  ...
  */
  ...
  def pathValueBothPathNotPopulated = eval( PathValue("4[1]", Operator.LT, "4[1]"), c1 ) === Pass
  ...
  def pathValuePathInvalid = {
    ...
    eval( p, c0 ) === Inconclusive(p, s"Invalid Path '${p.path1}'"::Nil)
  }
  ...
  def pathValuePathUnreachable = Seq(true, false) map { b =>
    ...
    eval( p, s0 ) === Inconclusive(p, s"Unreachable Path '${p.path1}'":: Nil)
  }
  ...
  def pathValueOnePathPopulated = {
    ...
    eval( p1, c1 ) === Failures.pathValueFailure(p1, `c1.3[1]`, "4[1]") and
    eval( p2, c1 ) === Failures.pathValueFailure(p2, `c1.3[1]`, "4[1]")
  }

  def pathValueManyElems = {
    val p = PathValue("2[*]", Operator.LT, "3[*]")
    val m = s"path1(2[*]) and path2(3[*]) resolution returned respectively 3 and 3 elements."
    eval( p, c1 ) === Inconclusive( p, m:: Nil )
  }
  ...
  def pathValueFail = {
    ...
    eval(p, c1) === Failures.pathValueFailure(p, `c1.1[1]`, `c1.3[1]`)
  }
