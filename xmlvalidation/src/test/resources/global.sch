...
<schema xmlns="http://www.ascc.net/xml/schematron">
    ...
    <title>Meaningful Use Rules for NCPDP 10.6 - Updated 2012-10-</title>
    ...
    <pattern  name="NEWRX_UIH-Errors" id="NEWRX_UIH-Errors">
        <rule context="script:Message">
            <assert test="@version='010'">ERROR: The Message/@version shall contain the value 010.
                See 170.314 (b)(3) Electronic Prescribing.
            </assert>
            <assert test="@release='006'">ERROR: The Message/@release shall contain the value 006.
                See 170.314 (b)(3) Electronic Prescribing.
            </assert>
        </rule>
    </pattern>
...
<!--  <pattern name="NEWRX_DRU-Errors" id="NEWRX_DRU-Errors">
    ...
    <pattern name="NEWRX-DRU-Errors-12-13-2011" id="NEWRX-DRU-Errors-12-13-2011">
		...
		<assert test="not(script:MedicationPrescribed/script:DrugCoded/script:DEASchedule)
			or
			(script:MedicationPrescribed/script:DrugCoded/script:DEASchedule = 'C48672')
		...
		<assert test="not(script:Refills/script:Qualifier)
			or
			(script:Refills/script:Qualifier = 'R')
				or (script:Refills/script:Qualifier = 'A')
				or (script:Refills/script:Qualifier = 'PRN')">
			ERROR: If /MedicationPrescribed/Refills/Qualifier is present, its value should be R or A or PRN.

		</assert>
		<!--  <assert test="script:Refills/script:Qualifier = 'PRN'
			and
			not(script:Refills/script:Value)">
			ERROR: If /MedicationPrescribed/Refills/Qualifier is PRN, then /MedicationPrescribed/Refills/Value must NOT be present.
	...
	<!-- <rule context="/script:Message/script:Body/script:NewRx/script:MedicationPrescribedPerson/script:Refills/script:Qualifier = 'PRN'">
    	<assert test="not(../script:Refills/script:Value)">ERROR: If /MedicationPrescribed/Refills/Qualifier is PRN, then /MedicationPrescribed/Refills/Value must NOT be present.</assert>
	</rule> -->
    </pattern>
</schema>